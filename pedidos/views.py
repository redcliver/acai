...
def local(request):
    if request.method == 'POST':
        cliente_obj = cliente.objects.filter(id=1).get()
...
def finalizar(request):
    ...
    if pedido_mix != None and tamanho1 != None and comanda_obj != None and adicionais1 == None: ##add MIX em comanda 
        ...
        novo_item = itemmix(mix_item=pedido_mix, total=total_pedido)
        novo_item.save()
        comanda_obj.total = comanda_obj.total + total_pedido
        comanda_obj.save()
    ...
    if pedido_casadinho != None and tamanho1 != None and comanda_obj != None and adicionais1 == None: ##add CASADINHO em comanda 
        ...
        novo_item = itemcasadinho(casadinho_item=pedido_casadinho, total=total_pedido)
        novo_item.save()
        comanda_obj.total = comanda_obj.total + total_pedido
        comanda_obj.save()
    ...
    if pedido_creme != None and tamanho1 != None and comanda_obj != None and adicionais1 == None: ##add CREME em comanda 
        ...
        novo_item = itemcreme(creme_item=pedido_creme, total=total_pedido)
        novo_item.save()
        comanda_obj.total = comanda_obj.total + total_pedido
        comanda_obj.save()
    ...
    if mix1 != None and adicionais1 != None and comanda_obj != None and pedido2 == None: ## add adicional em mix
        ...
        return render(request, 'pedidos/finalizar.html',{'title':'Fechamento', 'fondues1':fondues1, 'petits1':petits1, 'mshakes1':mshakes1, 'sorvetes1':sorvetes1, 'acais1':acais1, 'mixs1':mixs1, 'casadinhos1':casadinhos1, 'cremes1':cremes1, 'produtos1':produtos1, 'comanda_id':comanda_atual})
...
def cartao(request):
    ...
    nova_entrada = caixa_geral(total=novo_total, item=item_caixa, obs=metodo_caixa)
    nova_entrada.save()
    acais1 = comanda_atual.acais.all()
    mixs1 = comanda_atual.mixs.all()
    casadinhos1 = comanda_atual.casadinhos.all()
